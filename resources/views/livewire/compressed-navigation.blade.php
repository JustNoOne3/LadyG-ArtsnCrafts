                        ...
                        @foreach(App\Models\Brand::select('id', 'brand_logo')->get() as $idx => $brand)
                            <div class="flex block justify-center items-center min-w-0">
                                <a href="{{ route('shop.view', ['id' => $brand->id]) }}" style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;">
                                    <div x-data="{ loaded: false }" x-init="if ($el.querySelector('img').complete) { loaded = true }" class="flex items-center justify-center w-[100px] lg:w-[120px] h-20 lg:h-32">
                                        <!-- Skeleton Loader -->
                                        <div x-show="!loaded" class="w-full h-full bg-[#e6d9cb] animate-pulse rounded-xl"></div>
                                        <img 
                                            ...
                                            @load="console.log('Logo loaded:', $el.src); loaded = true"
                                        >
                                    </div>
                                </a>
                            </div>
